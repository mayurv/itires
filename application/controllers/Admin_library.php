...
class Admin_library extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model(array('users', 'backend/section_model', 'backend/payment'));
        ...
        $this->load->vars('includes_dir', base_url() . 'flexi_cart/includes/');
...
//        $this->load->view('admin', $this->data);
    ...
    function order_details($order_number) {
        ...
        $this->load->model(array('users', 'backend/pages_model'));
...
//        $this->template->write_view('content', 'admin/mng_product/_all_orders', $this->data);
...
//        $this->data['cart_items'] = $this->flexi_cart->cart_items();
        ...
        $this->template->write_view('content', 'admin/mng_product/_all_orders', $this->data, TRUE);
        ...
        $this->template->render();
//		$view = $this->load->view('home/order_detail', $this->data);
//        return $view;
    }

    function order_invoice($order_number) {
        if (!$this->ion_auth->logged_in()) {
            // redirect them to the login page
            redirect('auth/login', 'refresh');
        }
...
//		$view = $this->load->view('home/order_detail', $this->data);
    ...
    function invoice($orderId) {
        ...
        $this->data = $this->order_invoice($orderId);
...
//        $html = $this->load->view('invoice/invoice', $data, true, array(0, 0, 595, 841), 'a4', 'portrait');
        $html = $this->load->view('admin/mng_product/_invoice', $this->data, true, array(0, 0, 595, 841), 'a4', 'portrait');
...
//        echo $html;die;
    ...
    function all_order_details($order_number) {
        if (!$this->ion_auth->logged_in()) {
            // redirect them to the login page
            redirect('auth/login', 'refresh');
        }
        ...
        $this->data['page_title'] = 'All order summary';
        $this->data['total_users'] = count($this->users->get_all());
        $this->data['total_groups'] = count($this->group_model->get_all());
        // Get any status message that may have been set.
        $this->data['message'] = $this->session->flashdata('message');
...
//        $this->data['cart_items'] = $this->flexi_cart->cart_items();
        ...
        $this->template->write_view('header', 'backend/header', $this->data);
        $this->template->write_view('sidebar', 'backend/sidebar', NULL);
        $this->template->write_view('content', 'admin/mng_product/_all_orders_admin', $this->data);
        $this->template->write_view('footer', 'backend/footer', '', TRUE);
        $this->template->render();
//		$view = $this->load->view('home/order_detail', $this->data);
//        return $view;
    ...
    function item_discounts() {
        ...
        $sql_where = array(
            ...
            $this->flexi_cart_admin->db_column('discounts', 'type') => 2
        );
        $this->data['discount_data'] = $this->flexi_cart_admin->get_db_discount_array(FALSE, $sql_where);
        //print_r($this->data['discount_data']);die();
        ...
        $this->template->write_view('header', 'backend/header', $this->data);
        $this->template->write_view('sidebar', 'backend/sidebar', NULL);
        $this->template->write_view('content', 'admin/coupon/_view_coupon', $this->data);
        $this->template->write_view('footer', 'backend/footer', '', TRUE);
        $this->template->render();
...
//        $this->load->view('demo/admin_examples/discounts/discounts_view', $this->data);
    ...
    function update_discount($discount_id) {
        ...
        $this->template->write_view('header', 'backend/header', $this->data);
        $this->template->write_view('sidebar', 'backend/sidebar', NULL);
        $this->template->write_view('content', 'admin/coupon/_edit_coupon_view', $this->data);
        $this->template->write_view('footer', 'backend/footer', '', TRUE);
        $this->template->render();
     ...
     */
    function update_coupn($discount_id) {
        //   echo $discount_id;           die();

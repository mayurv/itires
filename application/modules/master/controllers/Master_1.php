...
//                            'createddate' => date('Y-m-d H:m:s'),
        ...
        $this->data['model_count'] = $this->mst_model->get_all();
        ...
        $totalRec = count($this->data['model_count']);
        /* Ajax Pagination */
...
//                'year_id' => array_search($this->input->post('year')[$i], $product_year),
                    ...
                    if ($yearArary != '') {
                        if (in_array($model['C'], $yearArary[0])) {
                            ...
                            $res = $this->checkIsModelExist($model['A'], $make_id, $dataSize['year_id']);

                            if ($res != 0) {
                                $dataSize['model_id'] = $res;
                            } else {
                                $model_id = $this->createModel($make_id, $dataSize['year_id'], $user_id, $model['A'], $dataSize['model_img_url']);

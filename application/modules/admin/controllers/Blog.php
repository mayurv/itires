...
//        $data['category_tree'] = $this->getBlogCategoriesTreeStructure();
        ...
        if ($this->input->post('search_null') == '1' && $posted_key == '') {
            $this->session->unset_flashdata('search');
        }
        if ($posted_key != '') {
...
//        $data['helpers'] = $this  ->common_model->getRecords('mst_users');
        ...
        if ($this->input->post('inputPostShortDescription') != '') {
            ...
            if ($_FILES['blog_image']['name'] != '') {
                ...
                $config['upload_path'] = 'media/backend/img/blog_image/';
                ...
                if (!$this->upload->do_upload('blog_image')) {
                    ...
                    $image_main = 'media/backend/img/blog_image/' . $blog_image;
                    ...
                    $image_main = 'media/backend/img/blog_image/' . $blog_image;
                    $thumbs_image = 'media/backend/img/blog_image/thumbs/' . $blog_image;
            ...
            if ($blog_image != '') {
                ...
                $blog_image = $this->input->post('blog_image_1');
            }
...
//                $data["arr_categories"] = $arr_categories;
                ...
                $this->data['blog_category'] = $this->blog_category->dropdown('category_name');
//                var_dump($this->data['blog_category']);die;
...
//        $this->load->view('front/includes/top-nav', $data);
    ...
    public function category($editid = NULL) {
//         echo $editid;die;
        if (!isset($editid)) {
            $user_id = $this->session->userdata('user_id');
            $this->data['dataHeader'] = $this->users->get_allData($user_id);
            ...
            $this->data['bcategory'] = $this->blog_category->as_array()->get_all();
            $this->template->set_master_template('template.php');
            $this->template->write_view('header', 'backend/header', $this->data);
            $this->template->write_view('sidebar', 'backend/sidebar', NULL);
            $this->template->write_view('content', 'blogs/_add_category', $this->data);
            $this->template->write_view('footer', 'backend/footer', '', TRUE);
            $this->template->render();
        } else {
            ...
            $this->data['bcategory'] = $this->blog_category->get_by_id($editid);
            $this->template->set_master_template('template.php');
            $this->template->write_view('header', 'backend/header', $this->data);
            $this->template->write_view('sidebar', 'backend/sidebar', NULL);
            $this->template->write_view('content', 'blogs/_edit_category', $this->data);
            $this->template->write_view('footer', 'backend/footer', '', TRUE);
            $this->template->render();
        }
    }

    public function add_cat($method = NULL, $catId = NULL) {

        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $method == "add") {
            $arrInsertData = array(
                'category_name' => $this->input->post('category_name'),
            );
            $this->blog_category->insert($arrInsertData);
...
//            var_dump($arrUpdateData);die;
            $this->blog_category->update($catId, $arrUpdateData);
    ...
    public function delete() {
        ...
        $result = $this->db->where('id', $id)->delete('it_blog_category');
        if ($result) {
            redirect('admin/blog/category');
        } else {
            echo '400';

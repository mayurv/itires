 ...
 */
//include('Flexi_cart_lite_model.php');
    ...
    private function calculate_reward_points() {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row_id => $reward_data) {
                ...
                // Set defaults.
                $data[$row_id]['total_points'] = $data[$row_id]['points_pending'] = $data[$row_id]['points_active'] = $data[$row_id]['points_expired'] = $data[$row_id]['points_converted'] = $data[$row_id]['points_cancelled'] = 0;
    ...
    private function update_discount_usage($discount_data = FALSE, $order_cancelled = FALSE, $operator = '-') {
        ...
        if ($this->get_enabled_status('discounts')) {
            ...
            $active_discount_data = array_merge(
                    $discount_data['settings']['discounts']['active_items'], $discount_data['settings']['discounts']['active_summary'], $discount_data['settings']['discounts']['reward_vouchers']
            );

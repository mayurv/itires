...
//        print_r($this->data['cart_items']);
        ...
        foreach ($this->data['cart_items'] as $k => $sData) {
            foreach ($itemId as $key => $data) {
                if ($data == $sData['id']) {
                    $this->data['cart_items'][$k]['quantity'] = $this->input->post('quantity_' . $sData['id']);
                }
            }
        }
        $cart_data = $this->data['cart_items'];
//        echo '<pre>adsad---------------';
//        print_r($this->data['cart_items']);
//        die;
    ...
    function demo_insert_ajax_link_item_to_cart($item_id = 0) {
        ...
        ###+++++++++++++++++++++++++++++++++###
        if ($this->input->post('quantity') && $this->input->post('quantity') != '')
...
//                'category_id' => $quantity,
        ...
        // Example #504.
            ...
            $name = $this->input->post('name');
            $price = $this->input->post('price');
            $img_url = $this->input->post('item_url');
            $quantity = $this->input->post('quantity');
            $option_1 = $this->input->post('option_1');
            $option_2 = $this->input->post('option_2');
...
//        redirect('lite_library/item_ajax_examples');
     ...
     */
    function demo_save_order($order_data) {
        $this->load->library('form_validation');
        $this->load->library('flexi_cart_admin');
        $order_data = $order_data['checkout'];
